var canvas = document.getElementById("cas"),
    ...
    vpy = canvas.height/2,
    Radius = 150,                		//整体大球的坐标
    LayerBallNum = 360 / 5,        // 横向
    LayerIntervalUp = 360 / 5;     //
    balls = [],
    angleX = Math.PI/500,
    angleY = Math.PI/500;
    ...
    layer.prototype = {
        setBalls: function (radius) {
            for(var i=0; i<LayerBallNum; i++){
                ...
                var b = new ball(radius * Math.cos(angle), radius * Math.sin(angle), this.up * Math.sqrt(Math.pow(Radius, 2) - Math.pow(radius, 2)), .5);
                b.paint();
                balls.push(b);
            }
